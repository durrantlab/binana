...
export var _get_hbonds = function (hydrogen_bonds, output) {
	var output = ((output + preface) + '') + '\n';
	...
	var output = ((output + preface) + '   -------------------|--------------------|---------------------|-------') + '\n';
	for (var key of sorted (hydrogen_bonds ['counts'].py_keys ())) {
		var value = hydrogen_bonds ['counts'] [key];
		var key = key.py_split ('_');
		var output = (((((((((output + preface) + '   ') + _center (key [1], 19)) + '|') + _center (key [2], 20)) + '|') + _center (key [3], 21)) + '|') + _center (str (value), 7)) + '\n';
	}
	var output = (((output + preface) + '\n') + preface) + 'Raw data:\n';
	for (var atom_pairs of hydrogen_bonds ['labels']) {
...
export var collect = function (parameters, ligand, closest, close, hydrophobics, hydrogen_bonds, salt_bridges, pi_pi, cat_pi, electrostatic_energies, active_site_flexibility, ligand_atom_types, json_output) {
	var output = '';
	...
	var output = _get_hbonds (hydrogen_bonds, output);
