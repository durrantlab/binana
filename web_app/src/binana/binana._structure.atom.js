...
import {r_just} from './binana._utils.shim.js';
...
export var Atom =  __class__ ('Atom', [object], {
	...
	get create_PDB_line () {return __get__ (this, function (self, index) {
		...
		var output = output + r_just (__mod__ ('%.3f', self.coordinates.x), 18);
		var output = output + r_just (__mod__ ('%.3f', self.coordinates.y), 8);
		var output = output + r_just (__mod__ ('%.3f', self.coordinates.z), 8);
