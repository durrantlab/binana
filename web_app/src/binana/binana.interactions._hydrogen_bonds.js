...
import {HYDROGEN_BOND_ANGLE_CUTOFF, HYDROGEN_BOND_DIST_CUTOFF} from './binana.interactions.default_params.js';
var __name__ = 'binana.interactions._hydrogen_bonds';
export var get_hydrogen_bonds = function (ligand, receptor, dist_cutoff, angle_cutoff) {
	...
	for (var [ligand_atom, receptor_atom, dist] of ligand_receptor_dists) {
		if (dist < dist_cutoff && (ligand_atom.element == 'O' || ligand_atom.element == 'N') && (receptor_atom.element == 'O' || receptor_atom.element == 'N')) {
			var hydrogens = [];
			for (var atm_index of ligand.all_atoms.py_keys ()) {
				if (ligand.all_atoms [atm_index].element == 'H' && ligand.all_atoms [atm_index].coordinates.dist_to (ligand_atom.coordinates) < 1.3) {
			...
			for (var atm_index of receptor.all_atoms.py_keys ()) {
				if (receptor.all_atoms [atm_index].element == 'H' && receptor.all_atoms [atm_index].coordinates.dist_to (receptor_atom.coordinates) < 1.3) {
