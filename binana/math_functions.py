...
def vector_scalar_multiply(vector, scalar):
    return binana.Point(vector.x * scalar, vector.y * scalar, vector.z * scalar)
...
def return_normalized_vector(vector):
    dist = distance(binana.Point(0, 0, 0), vector)
    return binana.Point(vector.x / dist, vector.y / dist, vector.z / dist)
