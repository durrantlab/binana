...
export var Mol =  __class__ ('Mol', [object], {
	...
	get load_pdb_from_text () {return __get__ (this, function (self, text_content, filename_to_use, min_x, max_x, min_y, max_y, min_z, max_z) {
		...
		for (var t = 0; t < len (lines); t++) {
			...
			if (line.__getslice__ (0, 3, 1) == 'END' && line.__getslice__ (0, 7, 1) != 'ENDROOT' && line.__getslice__ (0, 9, 1) != 'ENDBRANCH') {
				var t = textwrap.wrap (('WARNING: END or ENDMDL term found in ' + filename_to_use) + '. Everything after the first instance of this term will be ignored. If any of your files have multiple frames/poses, please partition them into separate files using (e.g., using vina_split) and feed each of the the single-frame files into BINANA separately.', 80);
				print ('\n'.join (t) + '\n');
				break;
			}
			...
			if (line.__getslice__ (0, 4, 1) == 'ATOM' || line.__getslice__ (0, 6, 1) == 'HETATM') {
				...
				if (!(self.has_hydrogens) && temp_atom.element == 'H') {
					self.has_hydrogens = true;
				}
				var temp_x = temp_atom.coordinates.x;
				var temp_y = temp_atom.coordinates.y;
				var temp_z = temp_atom.coordinates.z;
				if (temp_x > min_x && temp_x < max_x && temp_y > min_y && temp_y < max_y && temp_z > min_z && temp_z < max_z) {
					...
					var temp_residue = temp_atom.residue.strip ();
					var key = (((((temp_atom.atom_name.strip () + '_') + str (temp_atom.resid)) + '_') + temp_residue) + '_') + temp_atom.chain.strip ();
	...
	get warn_bad_atom_name () {return __get__ (this, function (self, py_name, residue) {
		self.printout (((('WARNING: There is no atom named "' + py_name) + '" in the protein residue ') + residue.strip ()) + '. Please use standard naming conventions for all protein residues to improve BINANA accuracy.');
		print ('');
	});},
	get check_protein_format_process_residue () {return __get__ (this, function (self, residue_atom_names, last_key) {
		var temp = last_key.strip ().py_split ('_');
		var resname = temp [0];
		var real_resname = resname.__getslice__ (-(3), null, 1);
		if (__in__ (real_resname, protein_resnames)) {
			if (!__in__ ('N', residue_atom_names)) {
				self.warn_bad_atom_name ('N', last_key);
			}
			if (!__in__ ('C', residue_atom_names)) {
				self.warn_bad_atom_name ('C', last_key);
			}
			if (!__in__ ('CA', residue_atom_names)) {
				self.warn_bad_atom_name ('CA', last_key);
			}
			for (var [residue_names, required_atom_names] of _required_protein_atom_names) {
				if (__in__ (real_resname, residue_names)) {
					for (var required_atom_name of required_atom_names) {
						if (!__in__ (required_atom_name, residue_atom_names)) {
	...
	get create_bonds_by_distance () {return __get__ (this, function (self) {
		var non_protein_atoms = self.non_protein_atoms.py_keys ();
		for (var atom_index1 of non_protein_atoms) {
			var atom1 = self.non_protein_atoms [atom_index1];
			if (__in__ (atom1.residue.__getslice__ (-(3), null, 1), protein_resnames)) {
				continue;
			}
			for (var atom_index2 of non_protein_atoms) {
	...
	get _categorize_hdonor_haccep_with_hydros () {return __get__ (this, function (self, atom, hydrogen_bond) {
	...
	get _is_hbond_donor_per_protein () {return __get__ (this, function (self, atom) {
		var atom_name = atom.atom_name.strip ();
		var resname = atom.residue.strip ();
		...
		return any ((function () {
			var __accu0__ = [];
			for (var [resnames, atom_names] of _protein_hydro_bond_donors) {
				__accu0__.append (__in__ (resname, resnames) && __in__ (atom_name, atom_names));
			}
			return py_iter (__accu0__);
		}) ());
	});},
	get _categorize_hydro_bond_donor_accep_no_hydros () {return __get__ (this, function (self, atom) {
		...
		if (is_protein_hbond_donor !== null) {
	...
	get assign_charged_from_protein_residue () {return __get__ (this, function (self, residue, last_key) {
		...
		var resname = temp [0];
		var real_resname = resname.__getslice__ (-(3), null, 1);
		if (__in__ (real_resname, _alternate_protein_resname ['LYS'])) {
		...
		if (__in__ (real_resname, _alternate_protein_resname ['HIS'])) {
			var charge_pt = Point (0.0, 0.0, 0.0);
			var count = 0.0;
			var indices = [];
			for (var index of residue) {
		...
		if (__in__ (real_resname, _alternate_protein_resname ['GLU'])) {
			var charge_pt = Point (0.0, 0.0, 0.0);
			var count = 0.0;
			var indices = [];
			for (var index of residue) {
		...
		if (__in__ (real_resname, _alternate_protein_resname ['ASP'])) {
			var charge_pt = Point (0.0, 0.0, 0.0);
			var count = 0.0;
			var indices = [];
			for (var index of residue) {
	...
	get assign_secondary_structure () {return __get__ (this, function (self) {
		...
		for (var atom_index of self.all_atoms.py_keys ()) {
			var atom = self.all_atoms [atom_index];
			if (__in__ (atom.residue.strip (), protein_resnames) && atom.atom_name.strip () == 'CA') {
			...
			for (var CA_atom_index of ca_list) {
				...
				if (CA_atom.structure == 'ALPHA') {
					...
					for (var other_CA_atom_index of ca_list) {
						...
						if (other_CA_atom.structure == 'ALPHA' && (other_CA_atom.resid - 3 == CA_atom.resid || other_CA_atom.resid + 3 == CA_atom.resid) && other_CA_atom.coordinates.dist_to (CA_atom.coordinates) < 6.0) {
							var another_alpha_is_close = true;
							break;
						}
					}
					if (!(another_alpha_is_close)) {
				...
				if (CA_atom.structure == 'BETA') {
					...
					for (var other_CA_atom_index of ca_list) {
						if (other_CA_atom_index != CA_atom_index) {
							var other_CA_atom = self.all_atoms [other_CA_atom_index];
							if (other_CA_atom.structure == 'BETA' && other_CA_atom.chain == CA_atom.chain && fabs (other_CA_atom.resid - CA_atom.resid) > 2 && CA_atom.coordinates.dist_to (other_CA_atom.coordinates) < 6.0) {
								var another_beta_is_close = true;
								break;
							}
						}
					}
					if (!(another_beta_is_close)) {
						self.set_structure_of_residue (CA_atom.chain, CA_atom.resid, 'OTHER');
						var change = true;
					}
				}
