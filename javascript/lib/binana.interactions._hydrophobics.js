...
import {HYDROPHOBIC_DIST_CUTOFF} from './binana.interactions.default_params.js';
import {_set_default} from './binana._utils.shim.js';
var __name__ = 'binana.interactions._hydrophobics';
export var get_hydrophobics = function (ligand, receptor, cutoff) {
	...
	for (var [ligand_atom, receptor_atom, dist] of ligand_receptor_dists) {
		...
		hydrophobic_labels.append (tuple ([ligand_atom.string_id (), receptor_atom.string_id (), dict ({'distance': dist})]));
	}
	return dict ({'counts': hydrophobics, 'mol': pdb_hydrophobic, 'labels': hydrophobic_labels});
};
